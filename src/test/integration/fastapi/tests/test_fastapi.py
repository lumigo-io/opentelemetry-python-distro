...
skip_attr_size_limit = pytest.mark.skipif(
    condition="OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT" in os.environ,
    reason="tests with default span attr max limit",
)


skip_no_attr_size_limit = pytest.mark.skipif(
    condition="OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT" not in os.environ,
    reason="tests with OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT set",
)


class TestFastApiSpans(unittest.TestCase):
    @skip_attr_size_limit
    ...
    def test_large_span_attribute_size_default_max_size(self):
        ...
        self.assertEqual(len(root["attributes"]["http.response.body"]), 2048)
        self.assertEqual(len(root["attributes"]["http.method"]), 3)
