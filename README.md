...
You can add execution tags to a span with dynamic values using the openTelemetry "vanilla" `set_attribute` method, with `lumigo.execution_tags.<key>` as the key.
...
get_current_span().set_attribute('lumigo.execution_tags.bar',('baz','xyz')); # Mind the parentheses, multiple values are passed as tuples in this example
```

**Note**: If you use the Span.set_attribute API multiple times to set multiple values, you may instead override the previous single value. Instead, use `Span.set_attribute(<key>, [<value_1>, <value_2>])` or `Span.set_attribute(<key>, (<value_1>, <value_2>))`
